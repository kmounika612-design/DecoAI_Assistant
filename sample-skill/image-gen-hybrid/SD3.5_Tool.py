...
cwd                   = os.getcwd()
context_binaries_path = os.path.join(cwd,"skills", "image-gen-hybrid", "Model_Bins")
LOCAL_MODEL_CACHE     = os.path.join(cwd,"skills", "image-gen-hybrid", "hf_model_cache")

user_step            = 20
tokenizer_max_length = 77
height, width        = 1024, 1024
user_seed            = np.int64(3)
user_text_guidance   = 8.5
...
def precompute_time_embeddings(time_text_embed, timesteps, pooled_uncond, pooled_cond):
    # Cast to float32 — text encoder outputs may be float16 but time_text_embed
    # linear layers require float32 inputs.
    pooled_uncond = pooled_uncond.float()
    pooled_cond   = pooled_cond.float()
    uncond_embeds, cond_embeds = [], []
    with torch.no_grad():
        for t in timesteps:
            ts = torch.tensor([t])
            uncond_embeds.append(time_text_embed(ts, pooled_uncond).numpy().astype(np.float32))
            cond_embeds.append(  time_text_embed(ts, pooled_cond  ).numpy().astype(np.float32))
...
_SERVER_PORT    = 50000
_SERVER_AUTHKEY = b"sd35-ort-server"
...
def get_te3_session(model_path: str):
    return get_session("text_encoder_3", model_path)
...
def run_text_encoder_T5(input_data, text_encoder_name, evaluate_perf=False):
    """Run T5 text encoder via ORT QNN EP. Returns prompt_embeds [1,seq,D]."""

    sess    = get_te3_session(_te_model_path('text_encoder_3'))
    outputs = _timed_run(sess, {sess.get_inputs()[0].name: input_data}, 'text_encoder_3')
    return outputs[0].reshape((1, -1, outputs[0].shape[-1]))


# ─── Transformer (ORT) ────────────────────────────────────────────────────────

# Pre-compute reshape constants once
_TR_PATH = os.path.join(context_binaries_path, "transformer_qnn_ctx_fp32_io.onnx")
_HW256   = int(height * width / 256)   # 4096
_H16     = int(height / 16)            # 64
_W16     = int(width  / 16)            # 64
_H8      = int(height / 8)             # 128
_W8      = int(width  / 8)             # 128
...
def run_transformer_batched(uncond_text, user_text, latent_in,
                            uncond_time_emb, cond_time_emb):
    """
    Run uncond + cond transformer in ONE batched call (batch_size=2).
    Halves the number of HTP inferences vs two separate calls.
    Returns (uncond_noise_pred, cond_noise_pred) both in NHWC.
    """
    sess = get_transformer_session(_TR_PATH)

    # Stack batch dim — all inputs already contiguous float32
    batch_text = np.concatenate([uncond_text, user_text], axis=0)          # (2, seq, D)
    batch_lat  = np.concatenate([latent_in,   latent_in], axis=0)          # (2, H, W, C)
    batch_time = np.concatenate([uncond_time_emb, cond_time_emb], axis=0)  # (2, D)
    outputs = _timed_run(sess, {
        "encoder_hidden_states_dq": batch_text,
        "hidden_states_dq":         batch_lat,
        "temb_dq":                  batch_time,
    }, 'transformer')

    out = outputs[0]  # (2, packed...)
    return _unpack_noise_pred(out[:1]), _unpack_noise_pred(out[1:])


def run_transformer(input_data_1, input_data_2, input_data_3, evaluate_perf=False):
    """Single-sample transformer call (used for evaluate_perf path)."""

    sess = get_transformer_session(_TR_PATH)
    outputs = _timed_run(sess, {
        "encoder_hidden_states_dq": np.ascontiguousarray(input_data_1, dtype=np.float32),
        "hidden_states_dq":         np.ascontiguousarray(input_data_2, dtype=np.float32),
        "temb_dq":                  np.ascontiguousarray(input_data_3, dtype=np.float32),
    }, 'transformer')
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate image using SD 3.5 medium model")
    ...
    user_prompt          = "A steaming bowl of rich beef noodle soup with tender beef, chewy noodles, and green onions. Steam rises into the neon-lit night market air. The scene is warm and inviting."#args.prompt
    user_negative_prompt = args.negative_prompt or ""
    #user_prompt = """Create an image of a sleek modern athletic shoe in vibrant orange and white, posed dramatically in front of the iconic Milan city at golden hour,
    #with Olympic rings softly glowing in the background sky, motion blur trails suggesting speed and peak athletic performance, professional sports photography style,
    #cinematic lighting, ultra detailed, photorealistic rendering. must include olympic rings""" """
    sys.path.insert(0, os.path.dirname(__file__))
    # from nsptargets import NspTargets  # noqa: F401
    ...
    # ── Time embedding (from local transformer weights) ───────────────────────
    time_text_embed = CombinedTimestepTextProjEmbeddings(
        embedding_dim=1536,       # SD3.5 Medium hidden size
        pooled_projection_dim=2048
    )
    time_text_embed.load_state_dict(torch.load(os.path.join(context_binaries_path, "time_text_embed_sd3.5_medium.pt")))
    ...
    uncond_tokens_t5 = run_tokenizer(user_negative_prompt, tokenizer3)
    cond_tokens_t5   = run_tokenizer(user_prompt,          tokenizer3)
    ...
    uncond_t5_prompt_embed = run_text_encoder_T5(uncond_tokens_t5, 'text_encoder_3')
    user_t5_prompt_embed   = run_text_encoder_T5(cond_tokens_t5,   'text_encoder_3')

    # ── Assemble embeddings (pure numpy — no torch overhead) ─────────────────
    def assemble_embeds(emb1, emb2, t5_emb):
        ...
        return np.concatenate([clip_padded, t5_emb.astype(np.float32), zeros], axis=1)

    uncond_text_emb = assemble_embeds(uncond_text_emb_1, uncond_text_emb_2, uncond_t5_prompt_embed)
    user_text_emb   = assemble_embeds(user_text_emb_1,   user_text_emb_2,   user_t5_prompt_embed)

    # ✅ Pooled embeddings: pure numpy concat, then wrap in torch once
    pooled_uncond_text_emb = torch.from_numpy(
        np.concatenate([pooled_uncond_text_emb_1, pooled_uncond_text_emb_2], axis=-1).astype(np.float32)
    )
    pooled_user_text_emb = torch.from_numpy(
        np.concatenate([pooled_user_text_emb_1, pooled_user_text_emb_2], axis=-1).astype(np.float32)
    )

    # ── Pre-compute time embeddings ───────────────────────────────────────────
    print("Pre-computing time embeddings ...", flush=True)
    uncond_time_embeds, cond_time_embeds = precompute_time_embeddings(
        time_text_embed, timesteps, pooled_uncond_text_emb, pooled_user_text_emb
    )
    del time_text_embed
    ...
    for i, t in enumerate(timesteps):
        ...
        uncond_noise = run_transformer(uncond_text_emb_c, latent_in, uncond_time_embeds[i])
        cond_noise   = run_transformer(user_text_emb_c,   latent_in, cond_time_embeds[i])
    ...
    # ── Save Output ───────────────────────────────────────────────────────────
    os.makedirs(r"C:\Users\HCKTest\.openclaw\media\hybrid-images", exist_ok=True)
    out_img_path = fr"C:\Users\HCKTest\.openclaw\media\hybrid-images\sd35_npu_output_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
