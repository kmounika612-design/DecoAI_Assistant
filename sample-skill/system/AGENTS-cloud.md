...
Use the `nano-banana-pro` skill for ALL image generation requests. This is a **skill, not a tool** — it will never appear in your tools list.

**How to invoke it:**
- The skill is triggered by sending a message that matches its activation pattern
- Do NOT attempt to call it as a function or tool call — it will fail
- Use it exclusively for image generation; do not use it for anything else
...
**When NOT to use it:**
- Web search, file operations, or any non-image task
- If the user asks for a diagram or chart — use text/markdown instead unless they explicitly want an image
