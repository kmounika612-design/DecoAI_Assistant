...
def _check_against_inventory(conn, item: DetectedItem) -> dict:
    """Compare one detected item to inventory stock.

    Matches by name+color (case-insensitive), falling back to name-only if the
    color doesn't match — a photo's perceived color often differs from the label.
    Returns the item plus in_stock / shortfall and a present|partial|missing status.
    """
    row = conn.execute(
        """SELECT id, quantity, bin_id FROM items
           WHERE lower(item_name) = lower(?)
             AND ifnull(lower(color), '') = ifnull(lower(?), '')""",
    ...
    if row is None:
        row = conn.execute(
            "SELECT id, quantity, bin_id FROM items WHERE lower(item_name) = lower(?)",
            (item.item_name,),
        ).fetchone()
    ...
    return {
        ...
        "shortfall": shortfall,
        "status": status,
        "bin_id": row["bin_id"] if row else None,
    }
